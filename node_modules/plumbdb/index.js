...
// TODO move keyseek and rangeseek into node-leveldb, refactor
PlumbDB.prototype._advanceKeySeek = function(key, iterator, readStream) {
  ...
  iterator.current(function(err, k, v) {
    ...
    readStream.emit('data', k.replace(me.docPrefix, ''), JSON.parse(v))
...
PlumbDB.prototype._advanceRangeSeek = function(endkey, iterator, readStream) {
  var me = this
  iterator.current(function(err, k, v) {
    if (k > endkey || !k) return readStream.emit('end')
    readStream.emit('data', k.replace(me.docPrefix, ''), JSON.parse(v))
    iterator.next(function(err) {
      me._advanceRangeSeek(endkey, iterator, readStream)
    })
  })
}

PlumbDB.prototype.rangeStream = function(startkey, endkey) {
  startkey = this.docPrefix + startkey
  endkey = this.docPrefix + endkey
  var me = this
  var readStream = new stream.Stream()
  this.db.iterator(function(err, iterator) {
    if (err) return readStream.emit('error', err)
    iterator.seek(startkey, function(err) {
      if (err) return readStream.emit('error', err)
      me._advanceRangeSeek(endkey, iterator, readStream)
    })
  })
